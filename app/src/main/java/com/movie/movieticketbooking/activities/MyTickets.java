...
import com.movie.movieticketbooking.listAdapters.CustomAdapter;
...
public class MyTickets extends ListActivity {
	...
	private static final  String EXPIRED_SECTION = "Session Expired";
	private static final  String VALID_SECTION = "Completed";
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		fetchMovieList(MovieParams.PAYMENT_HISTORY_API);
	}
	...
	private void fetchMovieList(String rssFeed) {
		...
		fetchTask.setOnCompleteListener(new NetworkTask.OnCompleteListener<List<BookingItems>>() {
			...
			public void onComplete(List<BookingItems> result) {
				...
				if(result!=null){
					...
					boolean isExpiredSection = false;
					List<BookingItems>	bList =new ArrayList<BookingItems>();
					SampleModel.getInstance().setbList(result);
					
					for (int i = 0; i < result.size(); i++) {
						if(result.get(i).transaction.equalsIgnoreCase(VALID_SECTION)){
							isValidSection = true;
						//	break;
						}
						if(result.get(i).transaction.equalsIgnoreCase(EXPIRED_SECTION)){
							isExpiredSection = true;
							//break;
						}
					}
					if(isValidSection){
					mAdapter.addSectionHeaderItem("VALID TICKETS");
					for (int i = 0; i < result.size(); i++) {
						if(result.get(i).transaction.equalsIgnoreCase(VALID_SECTION)){
							mAdapter.addItem(result.get(i).movieTitle);
						}
					}
					}					
					if(isExpiredSection){
						mAdapter.addSectionHeaderItem("EXPIRED TICKETS");
						for (int i = 0; i < result.size(); i++) {
							if(result.get(i).transaction.equalsIgnoreCase(EXPIRED_SECTION)){
								mAdapter.addItem(result.get(i).movieTitle);
							}
		...
		fetchTask.setOnGenericExceptionListener(new NetworkTask.OnExceptionListener() {
			...
			public void onException(Exception exception) {
				...
				Toast.makeText(MyTickets.this, MovieParams.MESSAGE_TXT, Toast.LENGTH_LONG).show();
			}
		});
		fetchTask.execute();
		}
